...
logger = None  # backward placeholder (avoid removal wide churn)
...
def parse_summary(html: str) -> Dict[str, Any]:
    ...
    log.debug(f"parse_summary meta={result['meta']}")
    return result
...
def get_next_page_url(html: str) -> Optional[str]:
    try:
        ...
        log.warn(f"next page html parse fail error={e}")
        return None
    try:
        ...
        if rel_next:
            ...
            if href:
                url = _abs_url(href)
                log.debug(f"next page strategy=rel_next url={url}")
                return url
    except Exception:
        pass
    try:
        ...
        for a in txt_candidates:
            ...
            if any(k in txt for k in ('次', '次へ', '次の')) and len(txt) <= 6:
                ...
                if href:
                    url = _abs_url(href)
                    log.debug(f"next page strategy=text_match text={txt} url={url}")
                    return url
    except Exception:
        pass
    try:
        ...
        if class_next:
            ...
            if href:
                url = _abs_url(href)
                log.debug(f"next page strategy=class_next url={url}")
                return url
    except Exception:
        pass
    try:
        ...
        if cur_num is not None:
            ...
            if num_links:
                ...
                if href:
                    url = _abs_url(href)
                    log.debug(f"next page strategy=number_follow current={cur_num} target={target} url={url}")
                    return url
    except Exception:
        pass
    ...
    for xp in static_xps:
        try:
            ...
            log.debug(f"next page xpath eval fail xp={xp} error={e}")
            continue
        if not nodes:
            continue
        ...
        if not href:
            continue
        url = _abs_url(href)
        log.debug(f"next page strategy=static xp={xp} url={url}")
        return url
    log.debug("next page not found")
    return None


# (The remainder: parse_cars and helpers) -- copied nearly verbatim for migration
ID_TR_PREFIX = "tr_"
ID_TD_PREFIX = "td_"

re_price = re.compile(r"([0-9]+(?:\.[0-9]+)?)\s*万")
re_price_plain = re.compile(r"^([0-9]+(?:\.[0-9]+)?)$")
...
def parse_cars(html: str) -> List[CarRecord]:
    ...
    for tr_node in tr_nodes:
        ...
        try:
            ...
            if not notes_raw:
                notes_raw = xp_first(card, './div[2]/div/div[2]/div[2]/div[2]/div[1]')
            if not notes_raw:
            ...
            log.warn(f"car extract fail tr_id={tr_id} error={e}")
            continue
    log.debug(f"parse_cars records={len(records)}")
    return records
